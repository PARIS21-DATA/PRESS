...
- `notebooks/` — Jupyter notebooks that implement the workflow.
  - `final/` — Production notebooks to run in order:
	 1. `A_title_pattern_matching.ipynb` — Applies purpose‑code filters and keyword rules to identify candidate projects from titles.
	 2. `colab_predict_with_finetuned_model.ipynb` (on Google Colab) — Uses a fine‑tuned large language model (LLM) to classify project descriptions.
	 3. `B_merge_predictions.ipynb` — Merges the rule‑based candidates with LLM predictions and harmonizes results.
	 4. `C_create_press.ipynb` — Builds the final PRESS outputs (tables, summaries, exports).
...
- Purpose codes: Projects are pre‑filtered using relevant CRS purpose codes.
- Keyword matching: Project titles are scanned with curated keyword lists (and translations) to catch likely candidates.
- LLM classification: A fine‑tuned large language model classifies the free‑text project descriptions to confirm or refute relevance.
