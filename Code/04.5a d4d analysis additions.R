...
#######
# 0. run this section one time every year
#######

# path_input_tmp <- paste0("data/intermediate/crs01_1_full_",
#                           year(Sys.Date()), 
#                           ".feather")
# path_output_tmp <- paste0("data/intermediate/04.5 d4d analysis ",
#                           year(Sys.Date()), 
#                           ".feather")
# df <-read_feather(path_input_tmp)
# gc()
# names(df)
# df_full <- df
# df <- df %>%
...
# write_feather(df, path_output_tmp)
# print_time_diff(start_time)
# rm(df_full)

path_input <- paste0("data/intermediate/04.5 d4d analysis ",
                     year(Sys.Date()), 
                     ".feather")
path_output <- paste0("data/intermediate/04.5a d4d manual additions ",
       year(Sys.Date()), 
       ".feather")
