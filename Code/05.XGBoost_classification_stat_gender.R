################################################################################
#
# XGBoost classification for gender statistical projects
...
if (iteration) { 
  pred_negative <- pred %>% 
    ...
    sample_n(size = neg_sample_fraction * df_crs %>% filter(stats_filter == TRUE & gender_filter == TRUE) %>% nrow)  %>% 
  ...
  pred <- df_crs %>%
    filter((stats_filter == FALSE | gender_filter == FALSE) | (is.na(stats_filter) | is.na(stats_filter)) & !(text_id %in% pred_negative$text_id)) %>%
    sample_n(size = floor(0.01 * n())) #use only 1% to speed up for testing
} else {
  pred <- df_crs %>%
...
train_data <- train_data %>% mutate(stats_gender_filter = ifelse(stats_filter & gender_filter, 1, 0))
test_data <- test_data %>% mutate(stats_gender_filter = ifelse(stats_filter & gender_filter, 1, 0))
pred <- pred %>% mutate(stats_gender_filter = ifelse(stats_filter & gender_filter, 1, 0))

label.train <- as.numeric(train_data$stats_gender_filter)
label.test <- as.numeric(test_data$stats_gender_filter)
label.prediction <- as.numeric(pred$stats_gender_filter)
...
if (print_importance_matrix) {
  ...
  pdf(paste0("./Tmp/XGBoost/Stat gender/importance_matrix_", it_add,"_", neg_sample_fraction, "_n", nrow(df), "test+train.pdf"))
...
pred_positive_text_id <- pred$text_id
saveRDS(pred_positive_text_id, file = "./Tmp/XGBoost/Stat gender/pred_postive_text_id.rds")
pred_stat_gender <- pred
saveRDS(pred_stat_gender, file = "./Tmp/XGBoost/Stat gender/pred_stat_gender.rds")
...
ggsave(paste0("./Tmp/XGBoost/Stat gender/word_distr_", it_add, "_", neg_sample_fraction,"_n", nrow(df),"test+train.pdf"), width = 9, height = 7)
...
ggsave("./Tmp/XGBoost/Stat gender/hist_donorname_train+test_data.pdf", width = 11, height = 7)
...
ggsave("./Tmp/XGBoost/Stat gender/hist_donorname_pred_data.pdf", width = 11, height = 7)
...
ggsave("./Tmp/XGBoost/Stat gender/hist_sector_train+test_data.pdf", width = 11, height = 7)
