...
# x <- df_crs_raw
x <- df_crs_output_original_names %>% 
...
e_fig$roll_byDonorType_bySpecific_value <- x %>% 
  filter(incomegroupname %in% c("LDCs", "LMICs",
                                "UMICs",
                                "Part I unallocated by income")) %>%
  mutate(incomegroupname = ifelse(incomegroupname %in% c("LDCs", "LMICs",
                                                         "UMICs"), 
                                  "Country Specific", 
                                  "Regional or unspecified")) %>% 
  mutate(group_year = year
         ,
         group_year =  (group_year - start_year) %/% roll_period
         , group_year = group_year* roll_period,
...
roll_period = 2
start_year = 2010
e_fig$roll_onlySpecific_bySpecificType_growth <- x %>% 
  filter(incomegroupname %in% c("LDCs", "LMICs",
                                "UMICs")) %>%
  mutate(group_year = year
         ,
...
e_fig$roll_onlySpecific_bySpecificType_value <- x %>% 
  filter(incomegroupname %in% c("LDCs", "LMICs",
                                "UMICs")) %>%
  mutate(group_year = year
         ,
...
#                                   incomegroupname)) %>% 
#   arrange(group_year) %>% 
#   group_by(incomegroupname) %>% 
#   mutate(group_year = ifelse(group_year == first(group_year), 
#                              paste0(group_year, "\n(baseline)"), 
#                              group_year)) %>% 
...
# 
# 
# e_fig$roll_byDonorType_bySpecific_value %>%
#   # rename(year = group_year) %>% 
#   arrange(group_year) %>% 
#   group_by(incomegroupname) %>% 
#   mutate(group_year = ifelse(group_year == first(group_year), 
...
#                              group_year)) %>% 
#   filter(donor_type != "Private") %>% 
#   ggplot( aes(x = group_year, y = total, group = donor_type)) +
#   geom_line(aes(linetype = donor_type, color = donor_type)) +
#   scale_linetype_manual(values = c("solid",
#                                    # "dotted",
#                                    "dashed")) +
...
e_fig$roll_byDonorType_bySpecific_value %>%
  ...
  # colnames
  rename(year = group_year
         , groups = incomegroupname) %>% 
  relocate(groups, .before = "year") %>% 
  arrange(groups, year)  %>% 
  write.xlsx("output/press/charts PRESS 2023/final_fig 1.3 by countryspecific by donortypes.xlsx")
...
e_fig$roll_byDonorType_byRecipientType_growth %>%
  ...
  mutate(group_year = ifelse(group_year == first(group_year), 
                             ...
                             group_year)) %>% 
  filter(donor_type != "Private"
         , incomegroupname != "Regional projects") %>% 
  rename(year = group_year
         , groups = incomegroupname) %>% 
  select(-baseline, -total) %>% 
  spread(key = donor_type, value = growth_rate) %>% 
  relocate(groups , .before = "year") %>% 
  arrange(groups, year)%>% 
  write.xlsx("output/press/charts PRESS 2023/final_fig 1.4 by incomegroup by donortypes.xlsx")



# 
# e_fig$roll_onlySpecific_bySpecificType_growth %>%
#   filter(donor_type != "Private") %>% 
#   arrange(group_year) %>% 
#   group_by(incomegroupname) %>% 
#   mutate(group_year = ifelse(group_year == first(group_year), 
...
#   facet_wrap(~incomegroupname 
#              # ,scales = "free_y"
#              ,ncol = 3) +  # Adjust ncol as needed
#   theme_minimal()

# e_fig$roll_onlySpecific_bySpecificType_value %>%
#   # rename(year = group_year) %>% 
#   arrange(group_year) %>% 
#   group_by(incomegroupname) %>% 
#   mutate(group_year = ifelse(group_year == first(group_year), 
...
#     color = "Donor Type"   # Change title for color legend
#     ,linetype = "Donor Type") + # Change title for linetype legend) +
#   facet_wrap(~incomegroupname, ncol = 3
#              # , scales = "free_y"
